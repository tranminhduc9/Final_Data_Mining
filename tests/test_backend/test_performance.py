...
def test_chat_latency(api_urls, auth_headers):
    ...
    # Đặt tiêu chuẩn nghiêm ngặt: Phải dưới 15 giây để đảm bảo trải nghiệm người dùng
    assert latency < 15, f"Chat is too slow for production: {latency:.2f}s (Target: < 15s)"

def test_concurrent_load(api_urls, auth_headers):
    """Giả lập tải 5 người dùng đồng thời."""
    def send_chat():
        try:
            s_res = requests.post(f"{api_urls['golang']}/chat/session", headers=auth_headers, timeout=5)
    ...
    for status in results:
        assert status in [200, 201], "System failed under concurrent load!"
